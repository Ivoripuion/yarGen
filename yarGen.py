...
def remove_long_strings(string):
    """
    去除字符串中过长的一些无意义字符串
    """
    if len(string) > 128:
        substrings = string.split("_")
        result = "_".join([s[:4] for s in substrings])
    else:
        result = string
...
def chinese_to_pinyin(string):
    ...
    result = ""
    for c in string:
        if has_chinese_character(c):
            pinyin_list = pinyin(c, style=Style.NORMAL, errors="ignore")
            if pinyin_list:
                result += pinyin_list[0][0]
            else:
                result += c
        else:
            result += c
    result = remove_long_strings(result)
    return result
    ...
    if not args.inverse:
        ...
        for filePath in file_strings:
            ...
            try:
                ...
                cur_rule_name = chinese_to_pinyin(cleanedName)
                file_name = chinese_to_pinyin(file)
                # Print rule title ----------------------------------------
                rule += "rule %s {\n" % cur_rule_name
        ...
        for super_rule in super_rules:
            try:
                ...
                if rule_name in printed_rules:
                    ...
                    printed_combi[rule_name] = 1
                rule_name = chinese_to_pinyin(rule_name)
                super_description = chinese_to_pinyin(file_listing)

                # Print rule title
                rule += "rule %s {\n" % rule_name
                rule += "   meta:\n"
                rule += "      description = \"%s - from files %s\"\n" % (prefix, super_description)
                rule += "      author = \"%s\"\n" % args.a
                rule += "      reference = \"%s\"\n" % reference
                rule += "      date = \"%s\"\n" % get_timestamp_basic()
    ...
    if args.inverse:
        ...
        for fileName in inverse_stats:
            try:
                ...
                inverse_rule_name = chinese_to_pinyin(cleanedName)
                inverse_description = chinese_to_pinyin(fileName)
                # Print rule title ----------------------------------------
                rule += "rule %s {\n" % inverse_rule_name

                # Meta data -----------------------------------------------
                rule += "   meta:\n"
                rule += "      description = \"%s for anomaly detection - file %s\"\n" % (prefix, inverse_description)
                rule += "      author = \"%s\"\n" % args.a
                rule += "      reference = \"%s\"\n" % reference
                rule += "      date = \"%s\"\n" % get_timestamp_basic()
